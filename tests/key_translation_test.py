...
from pycoin.encoding import bitcoin_address_to_hash160_sec, is_sec_compressed, \
    public_pair_to_sec, secret_exponent_to_wif, public_pair_to_bitcoin_address, \
    wif_to_tuple_of_secret_exponent_compressed, sec_to_public_pair, \
...
class BuildTxTest(unittest.TestCase):
    def test_translation(self):
        def do_test(exp_hex, wif, c_wif, public_pair_sec, c_public_pair_sec,
            ...
            self.assertEqual(
                secret_exponent_to_wif(secret_exponent,
                                       compressed=False),
                wif)
            self.assertEqual(
                secret_exponent_to_wif(secret_exponent,
            ...
            self.assertEqual(
                public_pair_to_sec(pk_public_pair,
                                   compressed=False),
                sec)
            ...
            self.assertEqual(
                ...
                public_pair_to_hash160_sec(pk_public_pair,
                                           compressed=True))
