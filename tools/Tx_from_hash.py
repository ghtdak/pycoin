...
def get_json_for_hash(the_hash):
    d = urllib.request.urlopen("http://blockexplorer.com/rawtx/%s" %
                               the_hash).read()
...
def main():
    ...
    for j_in in j.get("in"):
        txs_in.append(TxIn(
            h2b_rev(j_in["prev_out"]["hash"]), int(j_in["prev_out"]["n"]),
            tools.compile(j_in["scriptSig"])))

    txs_out = []
    for j_out in j.get("out"):
        txs_out.append(TxOut(
            int(float(j_out["value"]) * 1e8 + 0.5), tools.compile(j_out[
                "scriptPubKey"])))
