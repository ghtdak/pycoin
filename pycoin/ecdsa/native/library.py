...
def load_library():
    ...
    ECC_API = [
        ...
        ("EC_POINT_new", [ctypes.c_void_p],
         ctypes.c_void_p),  # TODO: make this a EC_POINT type
        ("EC_POINT_free", [ctypes.c_void_p], None),
        ("EC_POINT_set_affine_coordinates_GFp",
         [ctypes.c_void_p, ctypes.c_void_p, BN_P, BN_P, BN_CTX], ctypes.c_int),
        ("EC_POINT_get_affine_coordinates_GFp",
         [ctypes.c_void_p, ctypes.c_void_p, BN_P, BN_P, BN_CTX], ctypes.c_int),
        ("EC_POINT_mul", [ctypes.c_void_p, ctypes.c_void_p, BN_P,
                          ctypes.c_void_p, BN_P, BN_CTX], ctypes.c_int),
    ]
...
def make_fast_mul_f(library):
    ...
    def fast_mul(point, N):
        ...
        library.EC_POINT_set_affine_coordinates_GFp(NID_secp256k1_GROUP,
                                                    ec_point, bn_x, bn_y, ctx)

        library.EC_POINT_mul(NID_secp256k1_GROUP, ec_result, None, ec_point,
                             bn_n, ctx)

        library.EC_POINT_get_affine_coordinates_GFp(NID_secp256k1_GROUP,
                                                    ec_result, bn_x, bn_y, ctx)
