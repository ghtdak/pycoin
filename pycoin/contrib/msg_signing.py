...
def sign_message(key,
    ...
    try:
        r, s, y_odd = _my_sign(ecdsa.generator_secp256k1, secret_exponent,
                               mhash)
    except RuntimeError:
        # .. except if extremely unlucky
        k = from_bytes_32(os.urandom(32))
        r, s, y_odd = _my_sign(ecdsa.generator_secp256k1,
                               secret_exponent,
                               mhash,
                               _k=k)
    ...
    sig = b2a_base64(bytearray([first]) + to_bytes_32(r) + to_bytes_32(
        s)).strip()
...
def verify_message(key_or_address,
                   signature,
                   message=None,
                   msg_hash=None,
                   netcode=None):
...
def deterministic_make_k(generator_order,
                         secret_exponent,
                         val,
