...
def parse_as_public_pair(s):
    ...
    for c in ",/":
        if c in s:
            ...
            if v0:
                if s1 in ("even", "odd"):
                    return ecdsa.public_pair_for_x(ecdsa.generator_secp256k1,
                                                   v0,
                                                   is_even=(s1 == 'even'))
...
def main():
    ...
    for c in args.item:
        ...
        if public_pair:
            ...
            print("y parity:        %s" % "odd"
                  if (public_pair[1] & 1) else "even")
            print("key pair as sec: %s" % b2h(encoding.public_pair_to_sec(
                public_pair, compressed=True)))
