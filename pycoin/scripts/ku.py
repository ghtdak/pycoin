...
def gpg_entropy():
    try:
        output = subprocess.Popen(["gpg", "--gen-random", "2", "64"],
...
def create_output(item, key, subkey_path=None):
    ...
    if public_pair:
        ...
        add_output("key_pair_as_sec_uncompressed",
                   b2h(key.sec(use_uncompressed=True)),
                   " uncompressed")
    ...
    if hash160_u:
        ...
        add_output("address_uncompressed", address, "%s address uncompressed" %
                   network_name)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=('Known networks codes:\n  ' + ', '.join(['%s (%s)' % (
            i, full_network_name_for_netcode(i)) for i in NETWORK_NAMES])))
    ...
    for item in args.item:
        ...
        for key in key.subkeys(args.subkey or ""):
            ...
            if args.json:
                ...
                print(output_dict["wif_uncompressed"
                                  if args.uncompressed else "wif"])
            elif args.address:
                print(output_dict["address" + ("_uncompressed"
