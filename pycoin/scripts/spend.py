...
def secret_exponents_iterator(wif_file, private_keys):
    ...
    def private_key_iterator(pk):
        try:
            ...
            return (w.secret_exponent
                    for w in wallet.children(max_level=50,
                                             start_index=0))
...
def calculate_fees(unsigned_tx):
    total_value = sum(unsigned_tx_out.coin_value
                      for unsigned_tx_out in unsigned_tx.unsigned_txs_out)
...
def check_fees(unsigned_tx):
    ...
    if actual_tx_fee > recommended_tx_fee:
        ...
        print(
            "warning: transaction fee lower than (casually calculated) expected value of %s BTC, transaction might not propogate"
            % satoshi_to_btc(recommended_tx_fee))
...
def get_unsigned_tx(parser):
    ...
    for txinfo in args.txinfo:
        if '/' in txinfo:
            ...
            coins_sources = blockchain_info.unspent_tx_outs_info_for_address(
                txinfo)
...
def create_coinbase_tx(parser):
    ...
    try:
        if len(args.txinfo) != 1:
            parser.error(
                "coinbase transactions need exactly one output parameter (wif/BTC count)")
        wif, btc_amount = args.txinfo[0].split("/")
        satoshi_amount = btc_to_satoshi(btc_amount)
        secret_exponent, compressed = encoding.wif_to_tuple_of_secret_exponent_compressed(
            wif)
        public_pair = ecdsa.public_pair_for_secret_exponent(
            ecdsa.secp256k1.generator_secp256k1, secret_exponent)
        public_key_sec = encoding.public_pair_to_sec(public_pair,
...
def main():
    ...
    parser.add_argument('-g',
                        "--generate-unsigned",
                        help='generate unsigned transaction',
                        action='store_true')
    parser.add_argument('-f',
                        "--private-key-file",
                        help='file containing WIF or BIP0032 private keys',
                        metavar="path-to-file-with-private-keys",
                        type=argparse.FileType('r'))
    parser.add_argument('-p',
                        "--private-key",
                        help='WIF or BIP0032 private key',
                        metavar="private-key",
                        type=str,
                        nargs="+")
    parser.add_argument(
        '-c',
        "--coinbase",
        help='Create a (bogus) coinbase transaction. For testing purposes. You must include exactly one WIF in this case.',
        action='store_true')
    parser.add_argument(
        "txinfo",
        help='either a hex dump of the unsigned transaction, or a list of bitcoin addresses with optional "/value" if they are destination addresses',
        nargs="+")
    ...
    if args.coinbase:
        ...
        tx_output_index = 0
        tx_out_val = str(satoshi_to_btc(new_tx.txs_out[
            tx_output_index].coin_value))
        tx_out_script_hex = binascii.hexlify(new_tx.txs_out[
            tx_output_index].script)
        # product output in the form:
        #  tx_hash_hex/tx_output_index_decimal/tx_out_val/tx_out_script_hex
        # which can be used as a fake input to a later transaction
        print("/".join([tx_hash_hex, str(tx_output_index), tx_out_val,
                        tx_out_script_hex]))
        return
    ...
    secret_exponents = secret_exponents_iterator(args.private_key_file,
                                                 args.private_key)
