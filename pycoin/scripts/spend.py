...
def source_tx_for_address(bitcoin_address):
    r = json.loads(urllib.request.urlopen(
        "http://blockchain.info/unspent?active=%s" % bitcoin_address).read(
        ).decode("utf8"))
    tx_db = {}
    for unspent_output in r["unspent_outputs"]:
        tx_db[(binascii.unhexlify(unspent_output["tx_hash"]),
               unspent_output["tx_output_n"]
              )] = binascii.unhexlify(unspent_output["script"])
    return tx_db
...
def main():
    ...
    parser.add_argument('-d',
                        "--destination-address",
                        help='destination Bitcoin address/amount',
                        metavar='dest_address/amount',
                        nargs="+")
    parser.add_argument('-f',
                        "--secret-exponents",
                        help='WIF items for source Bitcoin addresses',
    ...
    new_tx = Tx.standard_tx(previous_hash_index__tuple_list,
                            coin_value__bitcoin_address__tuple_list, tx_db,
                            secret_exponents)
    s = io.BytesIO()
    new_tx.stream(s)
