...
def dump_tx(tx, netcode='BTC'):
    ...
    for idx, tx_out in enumerate(tx.txs_out):
        ...
        address = tx_out.bitcoin_address(
            address_prefix=address_prefix) or "(unknown)"
...
def parse_locktime(s):
    s = re.sub(r"[ ,:\-]+", r"-", s)
    for fmt1 in ["%Y-%m-%dT", "%Y-%m-%d", "%b-%d-%Y", "%b-%d-%y", "%B-%d-%Y",
                 "%B-%d-%y"]:
        for fmt2 in ["T%H-%M-%S", "T%H-%M", "-%H-%M-%S", "-%H-%M", ""]:
            ...
            try:
                ...
                if when < EARLIEST_DATE:
                    raise ValueError("invalid date: must be after %s" %
                                     EARLIEST_DATE)
