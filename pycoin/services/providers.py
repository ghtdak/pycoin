...
def spendables_for_address(bitcoin_address, netcode, format=None):
    ...
    for m in service_provider_methods(
            ...
            get_default_providers_for_netcode(netcode)):
        try:
...
def get_tx_db(netcode):
    ...
    return TxDb(lookup_methods=lookup_methods,
...
def bitcoin_rpc_init(match, netcode):
    username, password, hostname, port = match.group("user", "password",
                                                     "hostname", "port")
    return BitcoindProvider("http://%s:%s@%s:%s" %
