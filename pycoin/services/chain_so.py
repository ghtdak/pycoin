...
class ChainSoProvider(object):
    ...
    def __init__(self, netcode="BTC"):
        NETWORK_PATHS = {"BTC": "BTC", "XTN": "BTCTEST"}

        self.network_path = NETWORK_PATHS.get(netcode)

    def base_url(self, method, args):
        return "https://chain.so/api/v2/%s/%s/%s" % (method, self.network_path,
                                                     args)
    ...
    def unspents_for_address(self, address):
        ...
        """
        spendables = []
        r = json.loads(urlopen(self.base_url('get_tx_unspent', address)).read(
        ).decode("utf8"))
        ...
        for u in r['data']['txs']:
            ...
            spendables.append(Spendable(coin_value, script, previous_hash,
                                        previous_index))
