...
VERSION_PAIRS = [
    (True, False, 0x0488ADE4), (False, False, 0x0488B21E),
    (True, True, 0x04358394), (False, True, 0x043587CF)
]

VERSION_LOOKUP = dict(((private, test), v.to_bytes(4, byteorder="big"))
                      for private, test, v in VERSION_PAIRS)
VERSION_LOOKUP_REV = dict((v.to_bytes(4, byteorder="big"), (private, test))
                          for private, test, v in VERSION_PAIRS)
...
class Wallet(object):
    ...
    def __init__(self,
        ...
        if is_private:
            ...
            self.public_pair = ecdsa.public_pair_for_secret_exponent(
                ecdsa.generator_secp256k1, self.exponent)
        else:
    ...
    def fingerprint(self):
        return public_pair_to_ripemd160_sha_sec(self.public_pair,
                                                compressed=True)[:4]
    ...
    def subkey(self, i=0, is_prime=False, is_private=None):
        ...
        if is_private:
            exponent = (I_left_as_exponent + self.exponent
