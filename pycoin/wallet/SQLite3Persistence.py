...
as_text text not null
    ...
    def create_bip32node(self, slug, random_bytes):
        ...
        c = self._exec_sql("insert into BIP32Key (slug, as_text) values (?, ?)",
                           slug, bip32_text)
...
data text
    ...
    def set_global(self, slug, value):
        self._exec_sql("insert or replace into Global values (?, ?)", slug,
                       value)
...
unique(tx_hash, tx_out_index)
    ...
    def unspent_spendable_count(self):
        SQL = (
            "select count(*) from Spendable where block_index_available is not null"
