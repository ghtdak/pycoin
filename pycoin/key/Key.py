...
class Key(object):
    ...
    def __init__(self,
        ...
        """
        if [hierarchical_wallet, secret_exponent, public_pair, hash160].count(
                None) != 3:
            raise ValueError(
                "exactly one of hierarchical_wallet, secret_exponent, public_pair, hash160"
                " must be passed.")
    ...
    def from_text(class_, text, is_compressed=True):
        ...
        if key_type == 'wif':
            ...
            if is_compressed:
                data = data[:-1]
            return Key(secret_exponent=from_bytes_32(data),
                       prefer_uncompressed=not is_compressed,
                       netcode=netcode)
        if key_type == 'address':
            return Key(hash160=data,
    ...
    def public_copy(self):
        ...
        if self._hierarchical_wallet:
            return Key(
                hierarchical_wallet=self._hierarchical_wallet.public_copy())
        if self.public_pair():
    ...
    def _calculate_all(self):
        ...
        if self._secret_exponent:
            ...
            self._wif_compressed = secret_exponent_to_wif(self._secret_exponent,
                                                          compressed=True,
                                                          wif_prefix=wif_prefix)
    ...
    def as_text(self):
        ...
        """
        if self._hierarchical_wallet:
            return self._hierarchical_wallet.wallet_key(
                as_private=self._hierarchical_wallet.is_private)
        if self._secret_exponent:
            return self.wif()
        return self.address()
