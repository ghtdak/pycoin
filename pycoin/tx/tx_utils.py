    ...
    def get(self, v):
        ...
        for wif in self.wif_iterable:
            secret_exponent = wif_to_secret_exponent(
                wif,
                allowable_wif_prefixes=wif_prefix_for_netcode(self.netcode))
...
def distribute_from_split_pool(tx, fee):
    ...
    if zero_count > 0:
        total_coin_value = sum(spendable.coin_value
                               for spendable in tx.txs_in_as_spendable())
...
def sign_tx(tx, wifs=None, secret_exponent_db=None, netcode='BTC', **kwargs):
    """
...
def create_signed_tx(spendables,
    ...
    sign_tx(tx,
            wifs=wifs,
            secret_exponent_db=secret_exponent_db,
            netcode=netcode,
            **kwargs)
