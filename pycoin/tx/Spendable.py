...
class Spendable(TxOut):
    ...
    def from_dict(class_, d):
        return class_(
            d["coin_value"], h2b(d["script_hex"]), h2b(d["tx_hash_hex"]),
            d["tx_out_index"], d.get("block_index_available", 0),
    ...
    def __str__(self):
        return 'Spendable<%s mbtc "%s:%d">' % (satoshi_to_mbtc(self.coin_value),
