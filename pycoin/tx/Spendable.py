...
class Spendable(TxOut):
    ...
    def stream(self, f, as_spendable=False):
        ...
        if as_spendable:
            stream_struct("#LIbI", f, self.previous_hash, self.previous_index,
                          self.block_index_available, self.does_seem_spent,
    ...
    def from_dict(class_, d):
        return class_(d["coin_value"], h2b(d["script_hex"]),
                      ...
                      d.get("block_index_available"),
                      d.get("does_seem_spent", 0), d.get("block_index_spent"))

    def as_text(self):
        return "/".join(
            [b2h_rev(self.tx_hash), str(self.tx_out_index), b2h(self.script),
    ...
    def from_text(class_, text):
        ...
        return class_(coin_value, script, tx_hash, tx_out_index,
                      block_index_available, bool(does_seem_spent),
                      block_index_spent)
    ...
    def __str__(self):
        return 'Spendable<%s mbtc "%s:%d" %s/%s/%s>' % (
            satoshi_to_mbtc(self.coin_value), b2h_rev(self.tx_hash),
            self.tx_out_index, self.block_index_available,
            str(self.does_seem_spent)[0], self.block_index_spent)
