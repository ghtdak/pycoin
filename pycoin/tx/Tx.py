...
class TxIn(object):
    ...
    def __str__(self):
        return 'TxIn<%s[%d] "%s">' % (b2h_rev(self.previous_hash),
                                      self.previous_index,
                                      disassemble(self.script))
...
class TxOut(object):
    ...
    def __str__(self):
        return 'TxOut<%s "%s">' % (decimal.Decimal(self.coin_value) /
                                   COIN_FACTOR, disassemble(self.script))
...
class Tx(object):
    ...
    def coinbase_tx(class_, public_key_sec, coin_value, coinbase_bytes=b''):
        tx_in = TxInGeneration(previous_hash=bytes([0] * 32),
                               previous_index=(1 << 32) - 1,
                               script=coinbase_bytes)
    ...
    @classmethod
    def standard_tx(class_,
                    previous_hash_index__tuple_list,
                    coin_value__bitcoin_address__tuple_list,
                    tx_db=None,
                    secret_exponent_key_for_public_pair_lookup=None):
        # TODO: what is this?
        version = 1
        # TODO: what is this?
        lock_timestamp = 0
        tx_in_list = [TxIn(h, idx)
                      for h, idx in previous_hash_index__tuple_list]
        tx_out_list = []
        ...
        for coin_value, bitcoin_address in coin_value__bitcoin_address__tuple_list:
            ripemd160_sha = bitcoin_address_to_ripemd160_sha_sec(
                bitcoin_address)
    ...
    def validate(self, tx_db):
        for idx, tx_in in enumerate(self.txs_in):
            ...
            if not tx_from:
                raise ValidationFailureError("missing source transaction %s" %
                                             b2h_rev(tx_in.previous_hash))
            if tx_in.previous_hash != tx_from.hash():
                raise ValidationFailureError(
                    "source transaction %s has incorrect hash (actually %s)" %
                    (b2h_rev(tx_in.previous_hash), b2h_rev(tx_from.hash())))
            tx_out = tx_from.txs_out[tx_in.previous_index]
            if not verify_script(tx_in.script, tx_out.script, self, idx):
                raise ValidationFailureError(
                    "Tx %s TxIn index %d did not verify" %
                    (b2h_rev(tx_in.previous_hash), idx))

    def sign(self,
             tx_db,
             secret_exponents,
             public_pair_compressed_for_ripemd160_sha_key_lookup=None):
        ...
        if hasattr(secret_exponents, "get"):
            ...
            for secret_exponent in secret_exponents:
                ...
                secret_exponent_key_for_public_pair_lookup[
                    public_pair] = secret_exponent
                public_pair_compressed_for_ripemd160_sha_key_lookup[
                    public_pair_to_ripemd160_sha_sec(
                        public_pair, compressed=True)] = (public_pair, True)
                public_pair_compressed_for_ripemd160_sha_key_lookup[
                    public_pair_to_ripemd160_sha_sec(
                        public_pair, compressed=False)] = (public_pair, False)

        new_txs_in = []
        for tx_in in self.txs_in:
            tx_from = tx_db.get(tx_in.previous_hash)
            new_script = sign_signature(
                tx_from, self, tx_in.previous_index,
                secret_exponent_key_for_public_pair_lookup.get,
                public_pair_compressed_for_ripemd160_sha_key_lookup.get)
            if not new_script: raise Exception("bad signature")
            new_txs_in.append(TxIn(tx_in.previous_hash, tx_in.previous_index,
                                   new_script))
    ...
    def __repr__(self):
        return "Tx [%s] (v:%d) [%s] [%s]" % (self.id(), self.version, ", ".join(
            str(t) for t in self.txs_in), ", ".join(str(t)
                                                    for t in self.txs_out))
