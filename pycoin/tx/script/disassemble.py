...
from pycoin.tx import (SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE,
...
def add_signature_annotations(annotations, signature_blob,
    ...
    addresses = []
    pairs = possible_public_pairs_for_signature(generator_secp256k1, sig_hash,
                                                sig_pair)
    for pair in pairs:
...
def add_sec_annotations(a1, data, address_prefix):
    ...
    a1.append("SEC for %scompressed %s" %
              ("" if is_compressed else "un",
               public_pair_to_bitcoin_address(pair,
...
def annotation_f_for_scripts(input_script, output_script,
    ...
    def input_annotations_f(pc, opcode, data):
        ...
        if ld in (71, 72) and not is_p2sh:
            add_signature_annotations(a1, data, signature_for_hash_type_f,
                                      output_script)
        if ld == 20:
...
def disassemble_scripts(input_script, output_script, signature_for_hash_type_f):
    ...
    while pc < len(input_script):
        ...
        pre_annotations, post_annotations = input_annotations_f(pc, opcode,
                                                                data)
        yield pre_annotations, pc, opcode, instruction_for_opcode(
            opcode, data), post_annotations
        pc = new_pc

    pc = 0
    while pc < len(output_script):
        opcode, data, new_pc = get_opcode(output_script, pc)
        pre_annotations, post_annotations = output_annotations_f(pc, opcode,
                                                                 data)
        yield pre_annotations, pc, opcode, instruction_for_opcode(
            opcode, data), post_annotations
        pc = new_pc
    ...
    for r in disassemble_scripts(new_input_script, new_output_script,
                                 signature_for_hash_type_f):
        yield r
