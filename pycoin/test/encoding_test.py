...
class EncodingTestCase(unittest.TestCase):
    ...
    def test_to_from_long(self):
        ...
        def do_test(as_int, prefix, as_rep, base):
            self.assertEqual((as_int, prefix), encoding.to_long(
                base, encoding.byte_to_int, as_rep))
            self.assertEqual(as_rep, encoding.from_long(as_int, prefix, base,
                                                        lambda v: v))

        do_test(10000101, 2, binascii.unhexlify("00009896e5"), 256)
        do_test(10000101, 3, binascii.unhexlify("0000009896e5"), 256)
        do_test(
            1460765565493402645157733592332121663123460211377, 1,
    ...
    def test_to_from_base58(self):
        ...
        do_test("1abcdefghijkmnpqrst",
                b'\x00\x01\x93\\|\xf2*\xb9\xbe\x19b\xae\xe4\x8c{')
        do_test(
            "1CASrvcpMMTa4dz4DmYtAqcegCtdkhjvdn",
            b'\x00zr\xb6\xfac\xde6\xc4\xab\xc6\nh\xb5-\x7f3\xe3\xd7\xcd>\xc4\xba\xbd9')
        do_test(
    ...
    def test_wif_to_from_secret_exponent(self):
        ...
        def do_test(as_secret_exponent, as_wif, is_compressed):
            self.assertEqual(as_wif,
                             encoding.secret_exponent_to_wif(
                                 as_secret_exponent,
    ...
    def test_public_pair_to_sec(self):
        ...
        def do_test(as_public_pair, as_sec, is_compressed,
                    as_ripemd160_sha256_sec, as_bitcoin_address):
            self.assertEqual(
                encoding.public_pair_from_sec(as_sec), as_public_pair)
            self.assertEqual(
            ...
            self.assertEqual(
                encoding.public_pair_to_ripemd160_sha256_sec(
                    as_public_pair, compressed=is_compressed),
                as_ripemd160_sha256_sec)
            self.assertEqual(
                encoding.ripemd160_sha256_sec_to_bitcoin_address(
                    as_ripemd160_sha256_sec), as_bitcoin_address)
            self.assertEqual(
