...
class BlockHeader(object):
    ...
    def hash(self):
        if not hasattr(self, "__hash"):
            s = io.BytesIO()
            stream_struct("L##LLL", s, self.version, self.previous_block_hash,
    ...
    def __str__(self):
        return "BlockHeader [%s] (previous %s)" % (
            b2h_rev(self.hash()), b2h_rev(self.previous_block_hash))
...
class Block(BlockHeader):
    ...
    def __str__(self):
        return "Block [%s] (previous %s) [tx count: %d]" % (
            self.id(), b2h_rev(self.previous_block_hash), len(self.txs))
